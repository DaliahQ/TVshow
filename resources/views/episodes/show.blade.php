...
@section('content')
    <div class="container">
        ...
        @if ($episode->likes && $episode->likes->where('user_id', auth()->id())->count())
            <form action="{{ route('episodes.dislike', $episode->id) }}" method="POST" class="mt-4">
                @csrf
                @method('DELETE')
                <button class="bg-red-500 text-white px-4 py-2 rounded">Dislike</button>
            </form>
        @else
            <form action="{{ route('episodes.like', $episode->id) }}" method="POST" class="mt-4">
                @csrf
                <button class="bg-green-500 text-white px-4 py-2 rounded">Like</button>
            </form>
        @endif
    </div>
@endsection
